...
class GradeCourse
  ...
  def teachers(school_id = 2)
    ...
    ts << GradesCourse.where("book_id is not null").where(grade_id: g.id, course_id: course_id, is_accept: true).map{|g| User.find_by_id(g.teacher_id)} }.flatten.compact.uniq
  end
end
