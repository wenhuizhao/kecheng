...
module Tool
  ...
  module Percent
    ...
    def percent_between(obj, sdate, edate, int = false, opts = [])
      total = obj.homework_rang("#{sdate} 00:00:00", "#{edate} 23:59:59").where("grades_courses.is_open = 1 and (homeworks.created_at < '#{2.days.ago.to_s[0, 19]}' or homeworks.status = '1')")
